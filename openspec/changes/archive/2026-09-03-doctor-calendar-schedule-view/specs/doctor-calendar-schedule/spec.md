...
Schedule view SHALL keep Today, previous/next navigation, and the custom week picker available. Today SHALL reset the stream anchor to the current Singapore clinic date. Previous and next SHALL move the Schedule anchor by one preferred calendar week, and selecting a date or week from the custom picker SHALL re-anchor the stream to that selected date or week's first day. Re-anchoring SHALL reload from the new anchor without imposing a seven-day end boundary.
...
Schedule SHALL group appointments by their calendar date, omit date groups with no appointments, and order appointments within each group by start time and appointment identifier. Each appointment row SHALL show its time range, permitted administrative Patient display information, and readable lifecycle status. Schedule SHALL not expose clinical records, diagnoses, consultation notes, follow-up notes, prescriptions, work locations, or invented all-day event data.

#### Scenario: Doctor views a date group

- **WHEN** one or more appointments occur on a date in the loaded stream
...
- **WHEN** future appointments have different `AppointmentStatus` values, including CANCELLED
- **THEN** each row displays readable status text and distinguishable status treatment, with cancelled appointments retained but visibly muted
...
### Requirement: Schedule data and controls SHALL remain Doctor-owned and read-only

Schedule SHALL load only the signed-in Doctor's authorized administrative appointment projection. Viewing, scrolling, navigating, or changing the view mode SHALL not create, reschedule, cancel, check in, complete, check out, or otherwise mutate appointments or calendar settings. Schedule controls and states SHALL have accessible names or readable text.
...
- **WHEN** a Doctor uses the view selector, navigation controls, lazy-loading states, or appointment rows with keyboard navigation or assistive technology
